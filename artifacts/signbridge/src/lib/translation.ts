...
export async function sentenceToSignGloss(
  ...
  for (let i = 0; i < keys.length; i++) {
    try {
      return await _translateWithKey(text, keys[i]);
    } catch (e) {
      if (i === keys.length - 1) throw e;
      console.warn(`Translation key ${i + 1} failed, trying backup...`);
    }
  }
  throw new TranslationError("All API keys failed.", "NETWORK_ERROR");
}