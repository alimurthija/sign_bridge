...
function formatGeminiError(status: number, payload: unknown) {
  const message =
    ...
    typeof (payload as { error?: { message?: unknown } }).error?.message === "string"
      ...
      : "";
  if (status === 400) return `Gemini rejected the image request (400)${message ? `: ${message}` : "."}`;
  if (status === 403) return `Gemini rejected the API key (403)${message ? `: ${message}` : "."}`;
  if (status === 429) return `Gemini rate limit reached (429)${message ? `: ${message}` : "."}`;
  return `Gemini sign recognition failed with status ${status}${message ? `: ${message}` : "."}`;
}

async function _recognizeWithKey(base64: string, apiKey: string) {
...
const keys = [apiKey, ...envBackups].filter(Boolean);
  for (let i = 0; i < keys.length; i++) {
    try {
      ...
      console.warn(`API key ${i + 1} failed, trying backup...`);
    }
  }
